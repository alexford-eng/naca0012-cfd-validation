...
OpenFOAM has no 2D solver, so a 2D case is a one cell thick mesh with 'empty' boundary conditions on the two spanwise faces. The 3D NASA files are two identical planes separated by a unit distance. This is exactly the one cell thick mesh OpenFOAM requires; the 2D files would only give a surface without volume.
